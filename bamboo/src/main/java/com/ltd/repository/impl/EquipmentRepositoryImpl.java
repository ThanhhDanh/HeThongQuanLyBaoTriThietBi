...
//            }
            ...
            // Điều kiện thêm vào để lấy thiết bị có trạng thái "hoạt động"
            Predicate activeStatus = b.equal(b.lower(root.get("currentStatus")), "hoạt động".toLowerCase());
        ...
        if (params != null) {
            String pageParam = params.get("equipPage");
            int page = pageParam != null && !pageParam.isEmpty() ? Integer.parseInt(pageParam) : 0;
            if (page > 0) {
                PaginationUtils.applyPagination(query, page, 6);  // maxResults là 9
            }
        }
    ...
    public long countAllEquipments() {
        ...
        // Thêm điều kiện để chỉ đếm thiết bị hoạt động
        Predicate activeStatus = b.equal(b.lower(root.get("currentStatus")), "hoạt động".toLowerCase());
        q.select(b.count(root)).where(activeStatus);
